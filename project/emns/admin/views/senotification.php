...
$emergency='1';
 ...
 $sql500 = "select * from user where state='".$state."' and city = '".$city."' and area = '".$area."'";
...
$no = $no.' '.'7058320920';	
	...
	$numbers = array($no);
	$sender = urlencode('ANANDB');
	$message = rawurlencode('Notification from EMNS%n'.' '.$edesc);
...
$smpt=$conn->prepare("insert into sendno (edesc,state,city,area,fname,emergency) value(?,?,?,?,?,?)");
$smpt->bind_param("sssssi", $edesc,$state,$city,$area,$fname,$emergency);
